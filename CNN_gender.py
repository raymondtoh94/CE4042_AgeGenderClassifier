...
get_ipython().system("unzip '/content/drive/My Drive/Neural Networks/img_align_celeba.zip'")
...
wandb.init(project="gender-classifier 231120")
...
df = pd.read_csv("/content/drive/My Drive/Neural Networks/list_attr_celeba.csv")
...
tvtsplit = pd.read_csv("/content/drive/My Drive/Neural Networks/list_eval_partition.csv")
...
def build_model(IMG_WIDTH_HEIGHT, classes): # Gender Model
  model = Sequential()
  inputShape = (IMG_WIDTH_HEIGHT, IMG_WIDTH_HEIGHT, 3)
  chanDim = -1

  #Input Layer
  model.add(Conv2D(192, (4,4), padding="same", input_shape=inputShape))
  model.add(Activation("relu"))
...
# best_model = load_model('/content/model_93_percent_gender.h5')
best_model = load_model('/content/model.h5')
...
def load_img(img_path):
  ...
  img_batch = img_array.reshape(-1,100, 100, 3)
  # img_batch = img_array
  img_batch /= 255
